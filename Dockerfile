...
RUN apt-get -y install postgresql postgis postgresql-contrib postgresql-9.6-postgis-2.1 postgresql-server-dev-9.6
...
RUN service postgresql start && \
  pg_createcluster --start -e UTF-8 9.6 main

RUN sudo -u postgres /usr/lib/postgresql/9.6/bin/pg_ctl start -w -D /etc/postgresql/9.6/main/ && \
